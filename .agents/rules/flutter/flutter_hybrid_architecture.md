...
- Use `dart:js_interop` for web shell communication.
- Use `MethodChannel` / Pigeon for native mobile APIs (prefer Pigeon for type safety).
- Maintain a unified Riverpod state layer across all platforms.
- Use `Platform.isAndroid` / `kIsWeb` checks only at the infrastructure boundary.
