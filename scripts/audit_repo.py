...
def audit_repo(root_dir):
    ...
    # 0. Load Foundation Path to whitelist it
    foundation_path_val = None
    marker = root_path / ".agents" / ".foundation_path"
    if marker.exists():
        foundation_path_val = marker.read_text(encoding='utf-8').strip()
        print(f"📌 Whitelisting Foundation Path: {foundation_path_val}")
                ...
                try:
                    ...
                    for i, line in enumerate(lines, 1):
                        ...
                        for pattern in PATH_PATTERNS:
                            ...
                            if match:
                                found_path = match.group()
                                # WHITELIST: If the found path is exactly our foundation root, skip it
                                if foundation_path_val and foundation_path_val.lower() in found_path.lower():
                                    continue
                                    
                                print(f"[SANITY] Absolute path found in {path.relative_to(root_path)}:L{i} -> {found_path}")
                                issues_found += 1
